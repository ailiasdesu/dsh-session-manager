...
`;
		...
		function renderSessions(st, items) {
			...
			if (st.sessionsCount) st.sessionsCount.textContent = "会话 " + normal.length + (archived.length > 0 ? "（归档 " + archived.length + "）" : "");
			const visible = st.showArchived ? normal.concat(archived) : normal;
			if (visible.length === 0) {
				st.sessionsColBody.appendChild(emptyMsg(st.showArchived ? "没有归档会话" : "无会话"));
				return;
			}
			for (const item of visible) {
				...
				var isArchived = st.archivedIds.has(item.sessionId);
				...
				if (isArchived) main.appendChild(el("span", "sm-arch-badge", "已归档"));
		...
		function actBtn(label, onClick) {
			var b = el("button", "sm-act-btn", label);
			b.type = "button";
		...
		function buildSectionUI(ctx) {
			...
			var archivedToggle = el("button", "sm-toggle", "显示已归档 (0)");
			...
			var st = {
				...
				archivedToggle: archivedToggle, showArchived: false, archivedIds: new Set(),
				refreshBtn, disposed: false, diag: diagEl, confirmBar: confirmBar,
				pendingMigrate: null, sessionId: currentSessionId(ctx),
			};
			archivedToggle.addEventListener("click", () => {
				st.showArchived = !st.showArchived;
				archivedToggle.dataset.active = String(st.showArchived);
				void refreshAll(st);
			});
