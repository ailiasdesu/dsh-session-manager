...
.sm-ops{display:flex;gap:6px;margin-top:2px;opacity:0;transition:opacity .12s}
.sm-row:hover .sm-ops{opacity:1}
.sm-act-btn{border:1px solid var(--dsw-alias-stroke-default,#3a3a3a);background:#ffffff0d;color:inherit;border-radius:6px;padding:2px 9px;font-size:11px;cursor:pointer}
.sm-act-btn:hover{background:#ffffff1a}
.sm-act-danger{color:var(--dsw-alias-state-error-primary,#ff9b9b);border-color:#ff9b9b66}
.sm-act-danger:hover{background:#ff9b9b14}
.sm-toggle{margin-left:auto;border:1px solid var(--dsw-alias-stroke-default,#3a3a3a);background:transparent;color:inherit;border-radius:6px;padding:3px 10px;font-size:11px;cursor:pointer}
.sm-toggle[data-active="true"]{background:var(--dsw-alias-state-info-primary,#4c8dff);color:#fff;border-color:transparent}
.sm-arch-section{border-top:1px solid var(--dsw-alias-stroke-default,#3a3a3a)}
.sm-arch-body{max-height:220px;overflow-y:auto}
.sm-arch-badge{color:var(--dsw-alias-state-warn-primary,#ffd679);font-size:11px;border:1px solid #ffd67955;border-radius:5px;padding:0 5px;flex:none}
...
`;
		...
		async function refreshAll(st) {
			...
			try {
				...
				if (hostResp?.ok === true) {
					workspaces = hostResp.workspaces ?? [];
					st.archivedIds = new Set(hostResp.archivedSessionIds ?? []);
				}
			...
			const mains = items.filter((it) => it.origin !== "subagent");
			const normal = mains.filter((it) => !st.archivedIds.has(it.sessionId));
			const archived = mains.filter((it) => st.archivedIds.has(it.sessionId));
			if (st.sessionsCount) st.sessionsCount.textContent = "会话 " + normal.length;
			renderArchived(st, archived);
			if (normal.length === 0) {
			...
			for (const item of normal) {
				...
				if (!isCurrent) {
					ops.appendChild(actBtn("归档", async () => {
						var resp = await postApi({ sessionId: st.sessionId, action: "archive", targetId: item.sessionId });
						toast(st, resp?.ok === true ? "✅ " + resp.message : "❌ " + (resp?.message ?? "归档失败"), resp?.ok === true ? "ok" : "err");
						void refreshAll(st);
					}, false));
					ops.appendChild(actBtn("迁移", () => {
		...
		function renderArchived(st, archived) {
			if (!st.archivedBody) return;
			st.archivedBody.textContent = "";
			if (archived.length === 0) {
				st.archivedBody.appendChild(emptyMsg("无已归档会话"));
				return;
			}
			for (const item of archived) {
				...
				var main = el("div", "sm-row-main");
				main.appendChild(el("span", "sm-row-title", title));
				main.appendChild(el("span", "sm-arch-badge", "已归档"));
				row.appendChild(main);
				row.appendChild(el("div", "sm-row-sub", projectName(item.cwd ?? "")));
				var ops = el("div", "sm-ops");
				ops.appendChild(actBtn("恢复", async () => {
					var resp = await postApi({ sessionId: st.sessionId, action: "unarchive", targetId: item.sessionId });
					toast(st, resp?.ok === true ? "✅ " + resp.message : "❌ " + (resp?.message ?? "恢复失败"), resp?.ok === true ? "ok" : "err");
					void refreshAll(st);
				}, false));
				ops.appendChild(actBtn("彻底删除", async () => {
					var sure = window.confirm("⚠️ 彻底删除会话「" + shortId(item.sessionId) + "」？\n历史记录将被删除（备份保留在磁盘，可手工恢复）。此操作不可在 UI 撤销。");
					if (!sure) return;
					var resp = await postApi({ sessionId: st.sessionId, action: "delete", targetId: item.sessionId });
					toast(st, resp?.ok === true ? "✅ " + resp.message : "❌ " + (resp?.message ?? "删除失败"), resp?.ok === true ? "ok" : "err");
					void refreshAll(st);
				}, true));
				row.appendChild(ops);
				st.archivedBody.appendChild(row);
			}
		}

		function actBtn(label, onClick, danger) {
			var b = el("button", "sm-act-btn" + (danger ? " sm-act-danger" : ""), label);
			b.type = "button";
		...
		function buildSectionUI(ctx) {
			...
			toolbar.appendChild(el("span", "sm-title", "会话管理"));
			toolbar.appendChild(el("span", "sm-sub", "拖拽/点击迁移会话；「归档管理」处可恢复或彻底删除（全部操作带备份）"));
			...
			// Tab 栏：会话迁移 | 归档管理
			var tabBar = el("div", "sm-tabs");
			var tabMig = el("button", "sm-tab", "会话迁移");
			tabMig.type = "button"; tabMig.dataset.active = "true";
			var tabArch = el("button", "sm-tab", "归档管理");
			tabArch.type = "button";
			tabBar.append(tabMig, tabArch);
			// 迁移视图
			var cols = el("div", "sm-cols");
			var sessionsCol = el("div", "sm-col");
			var sessionsHead = el("div", "sm-col-head");
			var sessionsCount = el("span", "sm-count", "会话");
			sessionsHead.append(sessionsCount);
			sessionsCol.appendChild(sessionsHead);
			...
			// 归档视图（独立面板，Tab 切换）
			var archPanel = el("div", "sm-arch-panel");
			archPanel.hidden = true;
			archPanel.appendChild(el("div", "sm-col-head", "已归档会话"));
			var archivedBody = el("div", "sm-arch-body sm-arch-panel-body");
			archPanel.appendChild(archivedBody);
			root.append(toolbar, tabBar, diagEl, confirmBar, toastEl, cols, archPanel);

			var st = {
				root, toast: toastEl, toastTimer: null,
				sessionsColBody: sessionsBody, wsColBody: wsBody, sessionsCount: sessionsCount,
				archivedBody: archivedBody, archPanel: archPanel, archivedIds: new Set(),
			...
			function showTab(which) {
				var mig = which === "migrate";
				cols.hidden = !mig;
				archPanel.hidden = mig;
				tabMig.dataset.active = String(mig);
				tabArch.dataset.active = String(!mig);
				if (!mig) void refreshAll(st);
			}
			tabMig.addEventListener("click", () => showTab("migrate"));
			tabArch.addEventListener("click", () => showTab("archive"));
			uiState = st;
			void refreshAll(st);
			return st;
		}
		...
		// 插件契约
		// ---------------------------------------------------------------------------
		var name = "@dsh-external/dsh-session-manager";
