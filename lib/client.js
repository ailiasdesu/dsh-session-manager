...
`;
		...
		async function refreshAll(st) {
			...
			if (st.archivedToggle) st.archivedToggle.textContent = "已归档 (" + archived.length + ")";
		...
		function buildSectionUI(ctx) {
			...
			toolbar.appendChild(el("span", "sm-sub", "拖拽/点击迁移会话，管理归档（恢复、彻底删除）——全部操作带备份"));
			...
			var archivedToggle = el("button", "sm-toggle", "已归档 (0)");
			archivedToggle.type = "button";
			sessionsHead.append(sessionsCount, archivedToggle);
			sessionsCol.appendChild(sessionsHead);
			var sessionsBody = el("div", "sm-col-body");
			sessionsCol.appendChild(sessionsBody);
			var archivedSection = el("div", "sm-arch-section");
			var archivedHeadBtn = archivedToggle;
			var archivedBody = el("div", "sm-col-body sm-arch-body");
			archivedBody.hidden = true;
			archivedSection.appendChild(archivedBody);
			sessionsCol.appendChild(archivedSection);
			...
			var st = {
				...
				archivedToggle: archivedToggle, archivedBody: archivedBody, archivedIds: new Set(),
				refreshBtn, disposed: false, diag: diagEl, confirmBar: confirmBar,
				pendingMigrate: null, sessionId: currentSessionId(ctx),
			};
			archivedToggle.addEventListener("click", () => {
				archivedBody.hidden = !archivedBody.hidden;
				archivedToggle.dataset.active = String(!archivedBody.hidden);
			});
