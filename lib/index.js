 ...
 */
import { migrateFiles, rollbackFiles, projectKey, readHeaderSync, encodeSegment } from './migrate.js';
...
// ---------------------------------------------------------------------------
// 备份定期清理：保留期 BACKUP_RETENTION_DAYS（10 天），将超过保留期的
// 迁移/删除/索引备份目录整目录删除（目录名为 epoch 时间戳）
// ---------------------------------------------------------------------------
const BACKUP_RETENTION_DAYS = 10;
async function cleanupOldBackups(backupRoot, ctx) {
  try {
    ...
    function sweep(dir) {
      ...
      for (const entry of entries) {
        ...
        const ts = Number(entry.name);
        if (Number.isFinite(ts) && ts > 0) {
          if (ts < cutoff) { try { rmSync(p, { recursive: true, force: true }); removed++; } catch { /* 占用中跳过 */ } }
        } else {
          sweep(p); // 递归（如 <id>/、deleted/ 下的 id 层）
        }
      }
    }
    sweep(backupRoot);
    ctx?.logger?.info?.('[session-manager] backup cleanup: removed=' + removed + ' dirs (retention ' + BACKUP_RETENTION_DAYS + 'd)');
  } catch (e) { /* 静默 */ }
}
...
export function apply(ctx) {
  ...
  setTimeout(() => { void cleanupStaleSubagents(ctx, root); void cleanupOldBackups(backupRoot, ctx); }, 90000);
  setInterval(() => { void cleanupStaleSubagents(ctx, root); void cleanupOldBackups(backupRoot, ctx); }, 24 * 3600 * 1000);
