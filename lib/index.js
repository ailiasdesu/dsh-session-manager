...
export function apply(ctx) {
  ...
  async function doMigrate(targetId, targetPath) {
    ...
    try {
      ...
      try {
        await rollbackFiles({ root, id: targetId, srcCwd, newFile: moved.newFile });
        rollbackNote = '；文件已回滚';
      } catch { /* 保留备份提示 */ }
      try {
    ...
    return { ok: true, moved: true, message: movedTeams > 0 ? `已迁移到「${targetWs.title}」（含 ${movedTeams} 个团队状态）` : `已迁移到「${targetWs.title}」` };
  }
